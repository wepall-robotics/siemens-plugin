...
const PING_TIMEOUT_S = 1  # 1 segundo por intento

func ping_plc(selected_plc: PlcData) -> void:
    await get_tree().create_timer(1.0).timeout
    await _ping_thread(selected_plc)
...
func _ping_thread(selected_plc: PlcData) -> void:
    ...
    for attempt in range(MAX_PING_ATTEMPTS):
        # if attempt > 0:
        #     await get_tree().create_timer(PING_TIMEOUT_S).timeout
        # selected_plc.PingCompleted.connect(func(): print("Hola"))
        selected_plc.Ping()
        success = await selected_plc.ping_completed

        if success:
            break
            
        # Called when a ping attempt fails
        EventBus.ping_attempt_failed.emit(selected_plc, attempt + 1, MAX_PING_ATTEMPTS)
    
    if selected_plc:  # Verify that selected_plc is not null
        EventBus.ping_completed.emit(selected_plc, success)
